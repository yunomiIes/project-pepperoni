...
switch state
{
    ...
    case (189 << 0):
        ...
        if underground
        {
            ...
            visible = false
            if ((player.x > (x - 80)) && (player.x < (x + 80)) && (y <= (player.y + 60)) && (y >= (player.y - 60)) && ((player.state == (111 << 0)) || (player.sprite_index == player.spr_piledriverland) || ((player.state == (198 << 0)) && (player.vsp > 0))))
            {
                scr_soundeffect(sfx_enemies_treasureguy)
                underground = 0
                visible = true
                if ((player.x != x))
