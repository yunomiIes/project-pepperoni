...
if ((floor(image_index) == 10))
    shoving = 0
if ((room == strongcold_endscreen) || (room == rm_eggplantdisclaimer) || (room == rm_levelselect) || (room == rank_room) || (room == timesuproom) || (room == Realtitlescreen) || (room == characterselect) || (room == hub_loadingscreen) || (room == Scootertransition) || (room == boss_peppermanNEW) || (room == introfinalcutscene) || (room == LongintroREAL) || (room == menudark) || (room == longintroopening) || ((string_copy(room_get_name(room), 1, 5) == "tower") && (!global.panic)) || (string_copy(room_get_name(room), 1, 3) == "hub") || (string_copy(room_get_name(room), 1, 11) == "trickytreat"))
    visible = false
else
    visible = true
...
if (instance_exists(player) && (player.state != (64 << 0)) && (player.state != (89 << 0)) && (!follow_golf) && (!detach))
{
    if ((room != custom_lvl_room))
    {
        ...
        if ((!instance_exists(obj_fadeout)) && (global.smoothcamera == 1))
            camera_set_view_pos(view_camera[0], lerp(camera_get_view_x(view_camera[0]), cam_x, 0.2), (lerp(camera_get_view_y(view_camera[0]), cam_y, 0.2) + irandom_range((-shake_mag), shake_mag)))
        else
...
{
    global.lowfacefill--
    lowfacetimery = lerp(lowfacetimery, original_cam_height, 0.05)
    if (((global.lowfacefill % 60) == 0))
        scr_soundeffect(sfx_timercount)
}
else if (!instance_exists(obj_lowface))
{
    global.lowfacefill = time_in_frames(0, 10)
    lowfacetimery = (original_cam_height + 300)
}
