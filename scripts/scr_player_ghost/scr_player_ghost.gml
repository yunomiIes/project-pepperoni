...
function scr_player_ghost() //scr_player_ghost
{
    ...
    if ((move_v == 0))
        move_v = (key_down - key_up)
    if ((character == "NTV") && global.newnoisetransfoexp)
    {
        if ((hsp != 0))
            xscale = sign(hsp)
        if (!ghostdash)
    ...
    if ((sprite_index != spr_ghostend))
    {
        ...
        if ((character == "NTV") && global.newnoisetransfoexp)
        {
            ...
            if (key_slap2 && (ghostdashcooldown <= 0))
            {
                ...
                repeat (2)
                {
                    with (create_debris((x + random_range(-32, 32)), (y + random_range(-32, 32)), 2076, 1))
                    {
        ...
        if ghostdash
        {
            ...
            if ((character != "NTV") || ((character == "NTV") && (!global.newnoisetransfoexp)))
            {
                if ((ghostpepper == 1))
                    maxspeed = 8
                if ((ghostpepper == 2))
        ...
        else
        {
            ...
            else if ((character != "NTV") || ((character == "NTV") && (!global.newnoisetransfoexp)))
                m = 0
            ...
            if ((character == "NTV") && ghostdashstart && global.newnoisetransfoexp)
            {
                ghostdashstart = 0
                movespeed = dx
                vsp = dy
            ...
            if ((m != 0))
            {
                ...
                if ((character != "NTV") && scr_solid_player((x + sign(hsp)), y) && (!(place_meeting((x + sign(hsp)), (y + sign(vsp)), obj_slope))))
                    movespeed = 0
                if ((ghostbump < 0))
                {
                    hsp = (dx * ghostbump)
