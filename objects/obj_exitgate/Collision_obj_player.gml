...
with (obj_player)
{
    if (grounded && (x > (other.x - 160)) && (x < (other.x + 160)) && key_up && ((state == (0 << 0)) || (state == (103 << 0)) || (state == (104 << 0)) || (state == (121 << 0))) && ((global.panic == true) || (global.snickchallenge == 1) || (room == war_13)))
    {
        var ex = x
        var ey = y
        ...
        if ((global.combo > 0))
        {
            ...
            for (var i = 0; i < global.comboscore; i += 10)
                create_collect((obj_player1.x + irandom_range(-60, 60)), ((obj_player1.y - 100) + irandom_range(-60, 60)), choose(1597, 1599, 1600, 1602, 1598))
            global.comboscore = 0
        }
