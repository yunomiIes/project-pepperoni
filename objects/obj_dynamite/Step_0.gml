...
if (place_meeting((x + hsp), y, obj_solid) || place_meeting((x + hsp), (y + vsp), obj_destructibles) || ((obj_explosion != obj_canonexplosion) && place_meeting(x, y, obj_baddie)))
    image_xscale *= -1
if (place_meeting((x + hsp), (y + vsp), obj_destructibles) || place_meeting((x + hsp), (y + vsp), obj_metalblock) || place_meeting((x + hsp), (y + vsp), obj_ratblock) || ((obj_explosion != obj_canonexplosion) && place_meeting(x, y, obj_baddie)))
{
    instance_destroy()
    instance_create(x, y, obj_explosion)
}
