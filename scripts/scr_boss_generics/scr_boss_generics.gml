function scr_boss_grabbed() //scr_boss_grabbed
{
    ...
    with (playerid)
    {
        if ((state != (272 << 0)) || (baddiegrabbedID != other))
        {
...
function boss_hurt_gustavo() //boss_hurt_gustavo
{
    if pizzahead
    {
        ...
        {
            var _slam = (self.enemy_is_superslam(id) ? true : self.enemy_is_swingding(id))
            if ((thrown || _slam) && (other.state != (271 << 0)) && (place_meeting((x + hsp), y, other) || place_meeting((x - image_xscale), y, other) || place_meeting((x - (32 * image_xscale)), y, other) || place_meeting((x + (32 * image_xscale)), y, other)))
            {
                with (other)
                {
                    state = (138 << 0)
...
function scr_boss_do_hurt_phase2(argument0, argument1) //scr_boss_do_hurt_phase2
{
    ...
    self.fmod_event_one_shot("event:/sfx/misc/blackoutpunch")
    instance_create_unique(0, 0, 13)
    instance_create_unique(0, 0, 0)
...
function scr_boss_phase1hurt(argument0) //scr_boss_phase1hurt
{
    ...
    else if ((flashbuffer == 0))
    {
        ...
        with (player)
        {
            ...
            {
                depth = -12
                self.fmod_event_one_shot("event:/sfx/ending/star")
