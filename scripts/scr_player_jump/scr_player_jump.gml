function state_player_jump() //state_player_jump
{
    ...
    if ((sprite_index == spr_suplexbump) && (character == "PZ") && (!grounded))
        movespeed = 0
    if ((sprite_index == spr_suplexbump) && (character == "PZ") && grounded)
    {
        move = 0
    ...
    if grounded
    {
        if ((input_buffer_jump < 8) && (!key_down) && (!key_attack) && (vsp > 0) && (!(((sprite_index == spr_facestomp) || (sprite_index == spr_freefall) || (sprite_index == spr_player_pistolshot)))))
        {
            if ((character != "NTV"))
            ...
            if place_meeting(x, (y + 1), obj_railparent)
                railmomentum = 1
            if ((sprite_index != spr_shotgunshoot) && (sprite_index != spr_player_pistolshot))
            {
    ...
    if ((character != "S") && (character != "V"))
    {
        ...
        else if (key_slap2 && (!key_up) && (sprite_index != spr_suplexbump))
        {
            if ((character != "PI"))
            {
                sprite_index = spr_suplexdash
        ...
        else if (key_slap2 && key_up && (shotgunAnim == 0))
        {
            state = (80 << 0)
            image_index = 0
