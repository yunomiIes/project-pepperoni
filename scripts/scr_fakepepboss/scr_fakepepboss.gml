...
function scr_fakepep_init_sounds() //scr_fakepep_init_sounds
{
    ...
    snd_mach = undefined
    snd_superjump = undefined
    snd_grab = undefined
    snd_headoff = undefined
...
function scr_fakepepboss_deformed() //scr_fakepepboss_deformed
{
    ...
    if ((floor(image_index) < (image_number - 1)) && ((floor(image_index) div 5) == 0))
        create_debris((x + irandom_range(-20, 20)), (y + 43), 2212)
    if ((deformed_timer > 0))
        deformed_timer--
...
function scr_fakepepboss_grabdash() //scr_fakepepboss_grabdash
{
    ...
    else
    {
        ...
        {
            if ((!hurted) && (state != (268 << 0)) && (state != (91 << 0)) && (state != (146 << 0)) && (state != (272 << 0)))
            {
                other.state = (268 << 0)
                other.playerID = id
    ...
    else if place_meeting(x, (y + (sign(vsp) * yy)), obj_solid)
    {
        ...
        if ((superjumpbounce > 0) || (superjumpdir == 0))
        {
            ...
            if ((hitvsp > 0))
            {
                sprite_index = spr_fakepeppino_superjumpstart
                repeat (10)
                    create_debris((x + irandom_range(-25, 25)), (y + 43), 2212)
            ...
            else if ((hitvsp < 0))
            {
                ...
                repeat (10)
                    create_debris((x + irandom_range(-25, 25)), y, 2212)
